...
struct VerifyPhoneNumber: View {
  ...
  func verifyCode() {
    phoneViewModel.verifyCode(user: session.currentUser, isLoggedIn: session.isLoggedIn) {
    ...
    ZStack{
        ...
        VStack{
          ...
          HStack{
            ...
            VStack(alignment: .center, spacing: 15){
              ...
              Text("Code sent to \(phoneViewModel.phoneNumber)").font(.caption).foregroundColor(.gray)
            }
            Spacer()
      ...
      .alert(isPresented: $phoneViewModel.alertShown){
        Alert(title: Text("Error"),
              message: Text(self.phoneViewModel.alertMessage),
